...
// web listener on tcp port 20000 ~ 49000
while( !$wlistener ) {
	$wport = mt_rand( 20000, 49000 ) ;
	$wlistener = stream_socket_server("tcp://127.0.0.1:".$wport, $errno, $errstr);
}

$tunnelid = $wport ;

$wlistenfile = fopen( $session_path.'/sess_lvr_'.$tunnelid, "w" );
if( $wlistenfile ) {
	fprintf( $wlistenfile, "%d\r\n", $wport );
	fclose( $wlistenfile );
}
...
$recv_header = true ;
$xline = '' ;
$content_length = 1000000000 ;	// 1G max content length
while( $conn = stream_socket_accept($wlistener, $timeout ) ) {
...
@unlink( $session_path.'/sess_lvr_'.$tunnelid );
