...
<head>
	...
	<link href="tdclayout.css" rel="stylesheet" type="text/css" /><script src="https://code.jquery.com/jquery-<?php echo $jqver; ?>.js"></script><link href="jq/jquery-ui.css" rel="stylesheet" type="text/css" /><script src="jq/jquery-ui.js"></script>
	<script> if(window['jQuery']==undefined)document.write('<script src="jq/jquery.js"><\/script><link href="jq/jquery-ui.css" rel="stylesheet" type="text/css" \/><script src="jq/jquery-ui.js"><\/script>');</script><script src="picker.js"></script>
...
$("#requestlist").jqGrid({
    ...
    colModel :[ 
      {name:'vcq_vehicle_name', index:'vcq_vehicle_name', sortable: true, width:100}, 
      {name:'vcq_start_time', index:'vcq_start_time', width:150, sortable: true}, 
      {name:'duration', index:'duration', width:80, sortable: true , align:'right'}, 
      {name:'vcq_comp', index:'vcq_comp', width:100, sortable: true }, 
      {name:'vcq_ins_user_name', index:'vcq_ins_user_name', width:120, sortable: true }, 
      {name:'vcq_description', index:'vcq_description', width:240, sortable:false} 
    ],
    pager: '#requestpager',
	height: '380',
	width: '800',
...
$('#deleterequest').click(function(e){
	...
	$( ".tdcdialog#dialog_delete" ).data("yesfunction", function(){
		...
		fdata.vcq_id=id ;
		$.getJSON("vcqdel.php", fdata, function(resp){
			if( resp.res==1 ) {
...
$('form').submit(function() {
	$.getJSON("vcqsave.php", $('form').serializeArray(), function(resp){
		if( resp.res == 1 ) {
			$("#requestlist").trigger("reloadGrid");
		}
		else if( resp.errormsg ) {
...
$( "select[name='vcq_vehicle_name']" ).on( "change", function() {
	var vehicle = $( "select[name='vcq_vehicle_name']" ).val();
	...
	$.getJSON("cameralist.php", param, function(resp){
		...
		$( "select#vcq_camera" ).html( options );
...
<div><select name="vcq_vehicle_name" size="8" style="min-width:90%;"> <?php 
 ...
 <select id='vcq_camera' name="vcq_camera[]" multiple size="8" style="min-width:90%;"></select>
</fieldset>
...
<table>
<tr>
<td>From:</td><td><input class="datetimepicker" size="24" name="vcq_start_time" type="text" value="<?php $da=new DateTime(); $da->sub(new DateInterval('P1D')); echo $da->format('Y-m-d H:i:s'); ?>"/></td>
</tr>
<tr>
<td>Duration:</td><td><input name="vcq_duration" size="10" type="text" value="5" />minutes</td>
</tr>
</table>

</fieldset>

<p>Description: <br/><textarea name="vcq_description" cols="30" rows="8" maxlength="450" ></textarea></p>
...
<?php if( !empty($support_videoviacellular) ) { ?>
  ...
  <input   name="btset" checked="checked" href="videosrequestcell.php" id="btvideoreqcell" type="radio" /><label for="btvideoreqcell"> Request Video Clips Via Cellular </label>
<?php } else { ?>
